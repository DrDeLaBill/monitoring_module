...
{
#if STORAGE_DEBUG
	LOG_TAG_BEDUG(STORAGE_TAG, "get next of %lu address: begin\n", prev_addr);
#endif

    if (prev_addr + sizeof(storage_page_record_t) - 1 > STORAGE_SIZE) {
#if STORAGE_DEBUG
        LOG_TAG_BEDUG(STORAGE_TAG, "get next of %lu address: error - out of memory\n", prev_addr);
#endif
	...
	if (status != STORAGE_OK) {
#if STORAGE_DEBUG
		LOG_TAG_BEDUG(STORAGE_TAG, "get next of %lu address: error read errors list page - try to write errors list page\n", prev_addr);
#endif
	...
	if (status != STORAGE_OK) {
#if STORAGE_DEBUG
		LOG_TAG_BEDUG(STORAGE_TAG, "get next of %lu address: unable to read errors list page\n", prev_addr);
#endif
	...
	if (status != STORAGE_OK) {
#if STORAGE_DEBUG
		LOG_TAG_BEDUG(STORAGE_TAG, "get next of %lu address: unable to find storage start address\n", prev_addr);
#endif
		return STORAGE_ERROR;
	}
...
#if STORAGE_DEBUG
            LOG_TAG_BEDUG(STORAGE_TAG, "get next of %lu address: next address found  %lu\n", prev_addr, *next_addr);
#endif
            return STORAGE_OK;
        }
    }
...
#if STORAGE_DEBUG
    LOG_TAG_BEDUG(STORAGE_TAG, "get next of %lu address: there is no available next address\n", prev_addr);
#endif
    return STORAGE_ERROR_OUT_OF_MEMORY;
}
...
storage_status_t _storage_write_page(uint32_t page_addr, uint8_t* buff, uint16_t len, uint8_t appointment)
{
...
#if STORAGE_DEBUG
    LOG_TAG_BEDUG(STORAGE_TAG, "write page (address=%lu): begin\n", page_addr);
#endif

    if (len > sizeof(payload.v2.payload)) {
#if STORAGE_DEBUG
        LOG_TAG_BEDUG(STORAGE_TAG, "write page (address=%lu): buffer length out of payload size\n", page_addr);
#endif
        return STORAGE_ERROR_OUT_OF_MEMORY;
    }
...
#if STORAGE_DEBUG
        LOG_TAG_BEDUG(STORAGE_TAG, "write page (address=%lu): unavailable page address\n", page_addr);
#endif
        return STORAGE_ERROR_OUT_OF_MEMORY;
    }
...
#if STORAGE_DEBUG
		LOG_TAG_BEDUG(STORAGE_TAG, "write page (address=%lu): eeprom write error - eeprom busy\n", page_addr);
#endif
		return STORAGE_ERROR_BUSY;
	}
    if (eeprom_status != EEPROM_OK) {
#if STORAGE_DEBUG
        LOG_TAG_BEDUG(STORAGE_TAG, "write page (address=%lu): eeprom write error\n", page_addr);
#endif
        return STORAGE_ERROR;
    }
    if (storage_status != STORAGE_OK) {
#if STORAGE_DEBUG
        LOG_TAG_BEDUG(STORAGE_TAG, "write page (address=%lu): update errors list page error\n", page_addr);
#endif
        return STORAGE_ERROR;
    }
    ...
    if (eeprom_status == EEPROM_ERROR_BUSY) {
#if STORAGE_DEBUG
		LOG_TAG_BEDUG(STORAGE_TAG, "write page (address=%lu): eeprom read error - eeprom busy\n", page_addr);
#endif
		return STORAGE_ERROR_BUSY;
	}
    if (eeprom_status != EEPROM_OK) {
#if STORAGE_DEBUG
		LOG_TAG_BEDUG(STORAGE_TAG, "write page (address=%lu): error read record\n", page_addr);
#endif
		return STORAGE_ERROR;
	}
    if (!_storage_check_inverse_bits(&payload)) {
#if STORAGE_DEBUG
		LOG_TAG_BEDUG(STORAGE_TAG, "write page (address=%lu): bad inverse bits\n", page_addr);
#endif
		return STORAGE_ERROR_BITS;
	}

#if STORAGE_DEBUG
    LOG_TAG_BEDUG(STORAGE_TAG, "write page (address=%lu): OK\n", page_addr);
#endif

#if STORAGE_DEBUG
	util_debug_hex_dump(STORAGE_TAG, (uint8_t*)&payload, page_addr, sizeof(payload));
#endif
    return STORAGE_OK;
}
...
storage_status_t _storage_read_page(uint32_t page_addr, uint8_t* buff, uint16_t len, uint8_t appointment)
{
...
#if STORAGE_DEBUG
    LOG_TAG_BEDUG(STORAGE_TAG, "read page (address=%lu): begin\n", page_addr);
#endif

    if (len > sizeof(payload.v2.payload)) {
#if STORAGE_DEBUG
        LOG_TAG_BEDUG(STORAGE_TAG, "read page (address=%lu): buffer length out of payload size - %u > %d\n", page_addr, len, sizeof(payload.v2.payload));
#endif
        return STORAGE_ERROR_OUT_OF_MEMORY;
    }
    ...
    if (page_addr + sizeof(payload) - 1 > STORAGE_SIZE) {
#if STORAGE_DEBUG
        LOG_TAG_BEDUG(STORAGE_TAG, "read page (address=%lu): unavailable page address\n", page_addr);
#endif
        return STORAGE_ERROR_OUT_OF_MEMORY;
    }
...
#if STORAGE_DEBUG
        LOG_TAG_BEDUG(STORAGE_TAG, "read page (address=%lu): eeprom read error - eeprom busy\n", page_addr);
#endif
        return STORAGE_ERROR_BUSY;
    }
    if (status != EEPROM_OK) {
#if STORAGE_DEBUG
        LOG_TAG_BEDUG(STORAGE_TAG, "read page (address=%lu): eeprom read error\n", page_addr);
#endif
        return STORAGE_ERROR;
    }
    ...
    if (!_storage_check_inverse_bits(&payload)) {
#if STORAGE_DEBUG
        LOG_TAG_BEDUG(STORAGE_TAG, "read page (address=%lu): bad inverse bits\n", page_addr);
#endif
        return STORAGE_ERROR_BITS;
    }
...
#if STORAGE_DEBUG
        LOG_TAG_BEDUG(STORAGE_TAG, "read page (address=%lu): bad storage magic %08lX!=%08lX\n", page_addr, payload.v2.header.magic, STORAGE_PAYLOAD_MAGIC);
#endif
...
#if STORAGE_DEBUG
        LOG_TAG_BEDUG(STORAGE_TAG, "read page (address=%lu): bad storage version %i!=%i\n", page_addr, payload.v2.header.version, STORAGE_PAYLOAD_VERSION);
#endif
...
#if STORAGE_DEBUG
		LOG_TAG_BEDUG(STORAGE_TAG, "read page (address=%lu): wrong storage appointment %i!=%i\n", page_addr, payload.v2.header.appointment, appointment);
#endif
		return STORAGE_ERROR_APPOINTMENT;
	}

#if STORAGE_DEBUG
    LOG_TAG_BEDUG(STORAGE_TAG, "read page (address=%lu): OK\n", page_addr);
#endif
...
storage_status_t _storage_write_errors_list_page()
{
...
#if STORAGE_DEBUG
    LOG_TAG_BEDUG(STORAGE_TAG, "write errors list page: begin\n");
    LOG_TAG_BEDUG(STORAGE_TAG, "write errors list page: WARNING! Trying to write errors bits\n");
#endif
	...
	while (status != STORAGE_ERROR_OUT_OF_MEMORY) {
		...
		if (status != STORAGE_OK) {
#if STORAGE_DEBUG
			LOG_TAG_BEDUG(STORAGE_TAG, "write errors list page: unable to write errors list page (address=%lu)\n", page_addr);
#endif
	    ...
	    if (status != STORAGE_OK) {
#if STORAGE_DEBUG
	        LOG_TAG_BEDUG(STORAGE_TAG, "write errors list page: unable to check errors list page\n");
#endif
			page_addr += STORAGE_PAGE_SIZE;
			continue;
	    }
...
#if STORAGE_DEBUG
    LOG_TAG_BEDUG(STORAGE_TAG, "write errors list page on adddress %lu: OK\n", page_addr);
#endif
    return status;
}
...
storage_status_t _storage_read_errors_list_page()
{
    ...
    while (status != STORAGE_ERROR_OUT_OF_MEMORY) {
		...
		if (status != STORAGE_OK) {
#if STORAGE_DEBUG
			LOG_TAG_BEDUG(STORAGE_TAG, "read errors list page: unable to read\n");
#endif
			page_addr += STORAGE_PAGE_SIZE;
			continue;
		}
...
#if STORAGE_DEBUG
    	LOG_TAG_BEDUG(STORAGE_TAG, "read errors list page: error=%02x\n", status);
#endif
    }
...
#if STORAGE_DEBUG
            LOG_TAG_BEDUG(STORAGE_TAG, "bad byte %lu check: %d!=%d\n", i, payload->payload_bits[i], payload->invers_bits[i]);
#endif
            return false;
        }
    }
