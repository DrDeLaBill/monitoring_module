...
{
#if EEPROM_DEBUG
    LOG_BEDUG(EEPROM_TAG, "eeprom read: begin (addr=%lu, length=%u)\n", addr, len);
#endif

    if (addr + len > EEPROM_PAGE_COUNT * EEPROM_PAGE_SIZE) {
#if EEPROM_DEBUG
        LOG_BEDUG(EEPROM_TAG, "eeprom read: error - out of max address or length\n");
#endif
...
#if EEPROM_DEBUG
    LOG_BEDUG(EEPROM_TAG, "eeprom read: device i2c address - 0x%02x\n", dev_addr);
#endif
    ...
    if (status != HAL_OK) {
#if EEPROM_DEBUG
        LOG_BEDUG(EEPROM_TAG, "eeprom read: i2c error=0x%02x\n", status);
#endif
        return EEPROM_ERROR;
    }

#if EEPROM_DEBUG
    LOG_BEDUG(EEPROM_TAG, "eeprom read: OK\n");
#endif
...
{
#if EEPROM_DEBUG
    LOG_BEDUG(EEPROM_TAG, "eeprom write: begin (addr=%lu, length=%u)\n", addr, len);
#endif

    if (addr + len > EEPROM_PAGE_COUNT * EEPROM_PAGE_SIZE) {
#if EEPROM_DEBUG
        LOG_BEDUG(EEPROM_TAG, "eeprom write: error - out of max address or length\n");
#endif
        return EEPROM_ERROR_OOM;
    }
...
#if EEPROM_DEBUG
    LOG_BEDUG(EEPROM_TAG, "eeprom write: device i2c address - 0x%02x\n", dev_addr);
#endif
    ...
    if (status != HAL_OK) {
#if EEPROM_DEBUG
        LOG_BEDUG(EEPROM_TAG, "eeprom write: i2c error=0x%02x\n", status);
#endif
        return EEPROM_ERROR;
    }

#if EEPROM_DEBUG
    LOG_BEDUG(EEPROM_TAG, "eeprom write: OK\n");
#endif

    return EEPROM_OK;
}

uint32_t eeprom_get_pages_count()
{
	return EEPROM_PAGE_SIZE * EEPROM_PAGE_COUNT;
}
