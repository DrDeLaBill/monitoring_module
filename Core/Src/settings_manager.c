...
settings_status_t settings_reset() {
	LOG_TAG_BEDUG(STNG_MODULE_TAG, "SET DEFAULT SETTINGS\n");
...
settings_status_t settings_load() {
	...
	LOG_TAG_BEDUG(STNG_MODULE_TAG, "load settings: begin\n");
	uint32_t stor_addr = 0;
	storage_status_t status = storage_get_first_available_addr(&stor_addr);
	if (status != STORAGE_OK) {
		LOG_TAG_BEDUG(STNG_MODULE_TAG, "load settings: error storage access\n");
		return SETTINGS_ERROR;
	}

	status = storage_load(stor_addr, (uint8_t*)&buff, sizeof(buff));
	if (status != STORAGE_OK) {
		LOG_TAG_BEDUG(STNG_MODULE_TAG, "load settings: error read\n");
		return SETTINGS_ERROR;
	}

	if (!_settings_check(&buff)) {
		LOG_TAG_BEDUG(STNG_MODULE_TAG, "load settings: error check\n");
		return SETTINGS_ERROR;
	}

	memcpy((uint8_t*)&module_settings, (uint8_t*)&buff, sizeof(module_settings));
	LOG_TAG_BEDUG(STNG_MODULE_TAG, "load settings: end - OK\n");
	return SETTINGS_OK;
}


settings_status_t settings_save() {
	LOG_TAG_BEDUG(STNG_MODULE_TAG, "save settings: begin\n");

	if (!_settings_check(&module_settings)) {
		LOG_TAG_BEDUG(STNG_MODULE_TAG, "save settings: error check\n");
		return SETTINGS_ERROR;
	}
	...
	storage_status_t status = storage_get_first_available_addr(&stor_addr);
	if (status != STORAGE_OK) {
		LOG_TAG_BEDUG(STNG_MODULE_TAG, "save settings: error storage access\n");
		return SETTINGS_ERROR;
	}

	status = storage_save(stor_addr, (uint8_t*)&module_settings, sizeof(module_settings));
	if (status != STORAGE_OK) {
		LOG_TAG_BEDUG(STNG_MODULE_TAG, "save settings: error save %02x\n", status);
		return SETTINGS_ERROR;
	}

	LOG_TAG_BEDUG(STNG_MODULE_TAG, "save settings: end - OK\n");
	return SETTINGS_OK;
}

void show_settings()
{
	LOG_TAG_BEDUG(STNG_MODULE_TAG, LOG_BEDUG_SETTINGS_FORMAT);
}
